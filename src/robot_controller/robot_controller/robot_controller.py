...
class Controller(Node):
    ...
    def read_twist(self, msg, event=None) -> None:
        ...
        if abs(msg.linear.x) > 0.01:
            ...
            x_velo = 0.0
        print("read the twist message x linear velocity ")
        ...
        self.get_logger().info(f"Received cmd_vel: {msg.linear.x:.2f}")
    ...
    def pub_imu(self) -> None:
        ...
        if self.IMU:
            ...
            # new code
            self.get_logger().warn("IMU not initialized", throttle_duration_sec=5.0)
...
# change i2c baud to 40.00.0 khz
    def init_sensors(self):
        ...
        if self.simulate:
            ...
            self.light = None
            self.IMU = None
            self.get_logger().info("SIMULATE=True")
            return
    ...
    def read_sensors(self):
        ...
        while rclpy.ok():
            if self.bmp:
                ...
                self.sensor_data["temp"] = 0.0
                self.sensor_data["pressure"] = 0.0
                self.sensor_data["altitude"]  = 0.0
            if self.humidity_sensor:
                self.sensor_data["humidity"] = self.humidity_sensor.relative_humidity[0]
            else:
                self.sensor_data["humidity"] = 0.0
            if self.light:
                ...
                self.sensor_data["rgbw"] = [0, 0, 0, 0]
                self.sensor_data["gesture"] = 0
                self.sensor_data["prox"] = 0
            time.sleep(rate)
...
#                self.get_logger().info(str(e))
    ...
    def pub_mic(self):
        ...
        # new code
        mic_msg.data = float(self.sensor_data.get("mic", 0.0))
    ...
    def __init__(self):
        ...
        self.simulate = self.get_parameter("simulate")\
...
#        with open("/home/pi/heroswarmv2/ROS2/ros_ws/src/robot_controller/include/robots.json") as file:
        ...
        path_json = os.path.join(pkg_share, 'include', 'robots.json')
        try:
            ...
            self.get_logger().warn(
                f"robots.json not found at {path_json}; using parameter 'id'")
       ...
       # self.init_sensors()
        try:
            ...
            self.get_logger().warn(f"Sensors disabled: {e}")
